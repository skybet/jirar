...
if ( length(unique(tsTickets$workType)) > 1) {
  ...
  print(g1)
  g2 <- ggplot(filter(classOfWorkTickets, ! is.na(resolutionMonthDisplay)), aes(x=resolutionMonthDisplay, fill=workType)) + 
...
if ( length(unique(tsTickets$epicLink)) > 1) {
  ...
  g2 <- ggplot(filter(classOfWorkTickets, ! is.na(resolutionMonthDisplay)), aes(x=resolutionMonthDisplay, fill=epicLink)) + 
    geom_bar(stat="count") + theme(axis.text.x = element_text(angle=90, hjust=1)) + facet_wrap(~ epicLink) + 
