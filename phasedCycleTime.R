...
library(dplyr)
library(reshape2)
tsTickets <- read.csv("/Users/grovesro/Desktop/jiraR/jiraRDataset.csv", header=T, skipNul=T) %>%
  mutate(
...
summary <- tsTickets %>% 
  select( -created, -summary, -key, -resolution, -resolutionDate) %>% 
...
#summaryMelt$createdMonthDisplay <- factor(summaryMelt$createdMonthDisplay)
#, levels = summaryMelt[order(summaryMelt$createdMonth), "createdMonthDisplay"])
#factor(summaryMelt$createdMonthDisplay)
...
unique(summaryMelt[order(summaryMelt$createdMonth), "createdMonthDisplay"])
levels(summaryMelt$createdMonthDisplay)
